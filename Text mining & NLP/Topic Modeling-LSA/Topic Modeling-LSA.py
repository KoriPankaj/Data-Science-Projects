...
def load_articles(data_dir):
    ...
    for path in reuters.glob('*.sgm'):  # Standard Generalized Markup Language
        with path.open() as sgm_file:
            ...
            soup = BeautifulSoup(contents) # extract data(body) using beautifulsoup
            for article in soup.find_all('body'):
                yield article.text

def load_documents(document_dir):
    print(f'Loading from {document_dir}')
    documents = list(load_articles(document_dir)) # load the articles
    print(f'Loaded {len(documents)} documents')
    return documents

def prepare_documents(documents): 
    print('Preparing documents')
    documents = [clean(document) for document in documents] # clean each document
    documents = [preprocess_string(doc) for doc in documents] # preprocessing of documents
    return documents

def create_lsa_model(documents, dictionary, number_of_topics):
    print(f'Creating LSA Model with {number_of_topics} topics') 
    document_terms = [dictionary.doc2bow(doc) for doc in documents] # feature extraction = 1 document converted into bag of words
...
def run_lsa_process(documents, number_of_topics=10):
    ...
    lsa_model = create_lsa_model(documents, dictionary, number_of_topics) # running LSA for all the documents
